...
> My web app maily consists of labeled buttons and text fields with placeholders. It uses the menu selection and form fillin interaction styles. All of the buttons initiate some function— displaying a list in the box element or sending API requests. The text fields are marked with placeholders to reduce clutter on the interface. The box element on the right side of the page is marked with a header at the top based on the current state of the app: creating an invoice, viewing a list or single invoice, or displaying cutomer information. The overwhelming simplicity of the interface is intentional. The goal of this web app is to simplify the invoicing process, which is accomlished by the straight forward interface and the single-click creation and signature functions. Guidelines that this web app follows:  
> 1. Standardized tasks -> Creating an invoice requires the same actions everytime. Signing and returning an invoice is also consistent. 
...
> 4. Minimal memory load on the user -> Customer information autofills after their name is entered and there are buttons to display past invoices so the merchant does not have to memorize this information.
